---
title: "Publication List"
layout: single
permalink: /publication-list/
author_profile: true
---

**Conference Papers**

- **Algorithms for Deciding the Safety of States in Fully Observable Non-deterministic Problems**<br>
  _Proceedings of the 36th International Conference on Automated Planning and Scheduling (ICAPS 2026)_<br>
  <u>Johannes Schmalz</u>, Chaahat Jain<br>
  [[more](/fond-fault-analysis/)]

- **Probabilistic HTN Planning: Formalization and Computational Complexity Analysis**<br>
  _Proceedings of 22nd International Conference on Principles of Knowledge Representation and Reasoning (KR 2025)_<br>
  Mohammad Yousefi, <u>Johannes Schmalz</u>, Patrik Haslum, Pascal Bercher<br>
  [[pdf]({{ site.url }}/downloads/prob-htn/prob_htn_kr2025_paper.pdf)]

- **Solving Constrained Stochastic Shortest Path Problems with Scalarisation**<br>
  _Proceedings of the 28th European Conference on Artificial Intelligence (ECAI 2025)_<br>
  <u>Johannes Schmalz</u>, Felipe Trevizan<br>
  [[pdf]({{ site.url }}/downloads/carl/carl_extended_paper.pdf)] [[more](/carl/)]

- **Finding Optimal Deterministic Policies for Constrained Stochastic Shortest Path Problems**<br>
  _Proceedings of the 27th European Conference on Artificial Intelligence (ECAI 2024)_<br>
  <u>Johannes Schmalz</u>, Felipe Trevizan<br>
  [[pdf]({{ site.url }}/downloads/det-pi-for-cssp/det-pi-for-cssp_camera_ready.pdf)] [[more]](/det-pi-for-cssp/)

- **Efficient Constraint Generation for Stochastic Shortest Path Problems** <br>
  _Proceedings of the 38th AAAI Conference on Artificial Intelligence (AAAI 2024)_<br>
  <u>Johannes Schmalz</u>, Felipe Trevizan<br>
  [[pdf]({{ site.url }}/downloads/cgilao/cgilao_aaai24_paper.pdf)]  [[more](/cgilao/)]


**Journal Papers**

- **Efficient Constraint Generation for Stochastic Shortest Path Problems** <br>
  _Artificial Intelligence Journal (AIJ) 2026_<br>
  <u>Johannes Schmalz</u>, Felipe Trevizan<br>
  [[more](/cgilao/)]


**Workshop Papers**

- **Finding Plans and Heuristics with Spectral Graph Theory**<br>
  _Workshop on Heuristics and Search for Domain-independent Planning (HSDIP 2025)_<br>
  <u>Johannes Schmalz</u><br>
  [[pdf]({{ site.url }}/downloads/spectral-graph-theory/spectral_graph_theory_hsdip_paper_v2.pdf)] [[more](/spectral-graph-theory/)]